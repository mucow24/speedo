...
- **Index** (`--make-index`) — write `out/index.html`, a static landing page
  ...
  one palette. A route appears iff its `speed_map_<route>.html` is on disk; a
  `_google` companion, when present, is linked as a secondary engine.
...
| `out/` | Generated maps and the `--make-index` landing page (`index.html`). | no (the Google one may embed an API key) |
